...
(function() {
  ...
  generateParams = function(persistent, options) {
    var params, _ref;
    params = {
      env: ((_ref = options.env) != null ? _ref.split(',') : void 0) || []
    };
  ...
  initWatcher = function(config, callback) {
    var allConfigFiles, bower, component, exists, getFiles, watched, _ref;
    allConfigFiles = config._normalized.paths.allConfigFiles;
    _ref = config._normalized.packageInfo, bower = _ref.bower, component = _ref.component;
    ...
    return each(watched, exists, function(err, existing) {
      ...
      params = {
        ...
        usePolling: (_ref1 = config.watcher) != null ? _ref1.usePolling : void 0
      };
  ...
  generateCompilationLog = function(startTime, allAssets, generatedFiles, disposedFiles) {
    ...
    generatedFiles.forEach(function(generatedFile) {
      ...
      generatedFile.sourceFiles.forEach(function(sourceFile) {
        ...
        if (!isChanged && __indexOf.call(disposedFiles.generated, generatedFile) >= 0) {
  ...
  getPlugins = function(packages, config) {
    return packages.filter(function(plugin) {
      ...
      if (worker.isWorker && ((_ref = config.workers) != null ? _ref.extensions : void 0)) {
        if (_ref1 = (_ref2 = plugin.prototype) != null ? _ref2.extension : void 0, __indexOf.call(config.workers.extensions, _ref1) < 0) {
          return false;
        }
      }
      return ((_ref3 = plugin.prototype) != null ? _ref3.brunchPlugin : void 0) && (!worker.isWorker || ((_ref4 = plugin.prototype) != null ? _ref4.compile : void 0) || ((_ref5 = plugin.prototype) != null ? _ref5.lint : void 0));
    }).map(function(plugin) {
      var instance;
  ...
  initialize = function(options, configParams, onCompile, callback) {
    ...
    return helpers.loadConfig(options.config, configParams, function(error, config) {
      var callCompileCallbacks, callbacks, compilers, fileList, joinConfig, launchWatcher, linters, optimizers, packages, plugins, server;
      ...
      packages = (loadPackages('.')).filter(function(_arg) {
        var brunchPluginName, _ref, _ref1;
        brunchPluginName = _arg.brunchPluginName;
        if (((_ref = config.plugins.off) != null ? _ref.length : void 0) && __indexOf.call(config.plugins.off, brunchPluginName) >= 0) {
          return false;
        } else if (((_ref1 = config.plugins.only) != null ? _ref1.length : void 0) && __indexOf.call(config.plugins.only, brunchPluginName) < 0) {
          return false;
        } else {
          return true;
        }
      });
      plugins = (getPlugins(packages, config)).filter(function(plugin) {
        var isOptimizer, _ref, _ref1, _ref2;
        if (((_ref = config.plugins.on) != null ? _ref.length : void 0) && (_ref1 = plugin.brunchPluginName, __indexOf.call(config.plugins.on, _ref1) >= 0)) {
          plugin.defaultEnv = '*';
        }
        ...
        isOptimizer = typeof plugin.optimize === 'function';
        if (plugin.defaultEnv == null) {
          plugin.defaultEnv = !config.optimize && isOptimizer ? 'production' : '*';
        }
        return plugin.defaultEnv === '*' || (_ref2 = plugin.defaultEnv, __indexOf.call(config.env, _ref2) >= 0);
      });
  ...
  bindWatcherEvents = function(config, fileList, compilers, linters, watcher, reload, onChange) {
    var bowerConfig, changeHandler, packageConfig, possibleConfigFiles, _ref;
    possibleConfigFiles = config._normalized.paths.possibleConfigFiles;
    _ref = config.paths, packageConfig = _ref.packageConfig, bowerConfig = _ref.bowerConfig;
    ...
    function BrunchWatcher(persistent, options, onCompile) {
      ...
      initialize(options, configParams, onCompile, (function(_this) {
        return function(error, result) {
          var compile, compilers, fileList, includes, linters, reload, watcher, watcherReady, _ref;
          if (error != null) {
            return logger.error(error);
          }
          ...
          if ((_ref = _this.config.workers) != null ? _ref.enabled : void 0) {
            if (!worker({
